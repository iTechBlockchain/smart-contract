...
contract HTLCBridge is Ownable {
    ...
    modifier noActiveTransferOut() {
        require(
            ...
            "Error: Ongoing Transfer, wait until it either completes or expires"
        );
        _;
    }
    ...
    function portalFromOtherChain(
        ...
        require(
            contractToContract[_tokenContract] != address(0x0),
            "Error: Token contract doesn't have a match in this chain"
        );
        _transfersIn[_receiver] = Transfer(
            _commitment,
            _sender,
            _receiver,
            contractToContract[_tokenContract],
            _amount,
            _hashLock,
            _timeLock
        );
    ...
    function finalizeInterPortalTransferDest(
        address _receiver,
        string memory _secretKey
    ) public {
        Transfer memory transfer = _transfersIn[_receiver];
        IERC20 tokenContract = IERC20(
            contractToContract[transfer.tokenContract]
        );
        require(
            hashThis(abi.encode(_secretKey)) == transfer.hashLock,
            "Error: hash lock does not match"
        );
        require(
            block.timestamp <= transfer.timeLock,
            "Error: transfer wasn't finalized within time"
        );
        require(
            tokenContract.balanceOf(address(this)) >= transfer.amount,
            "Error: not enough liquidity to bridge funds"
        );
    ...
    function withdrawFunds() public {
        require(
            _hasActiveTransferOut[msg.sender],
            "Error: sender does not have a pending transfer"
        );
    ...
    function getTransferOut(
        ...
        external
        view
        returns (bytes32, address, address, address, uint, uint, bytes32)
    {
        require(
            _hasActiveTransferOut[_sender],
            "Error: There aren't any ongoing transfer for the sender"
        );
        ...
        return
            hashThis(
                abi.encodePacked(
                    hashThis(
                        abi.encodePacked(
                            hashThis(abi.encodePacked(_sender)),
                            hashThis(abi.encodePacked(_receiver))
                        )
                    ),
                    hashThis(
                        abi.encodePacked(
                            hashThis(abi.encodePacked(_tokenContract)),
                            hashThis(abi.encodePacked(_amount))
                        )
                    )
                )
            );
